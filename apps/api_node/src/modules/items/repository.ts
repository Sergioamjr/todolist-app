...
export class LibsqlItemRepository implements IItemRepository {
  async findAll(userId: string, filters: ItemFilters = {}) {
    ...
    return result.rows.map((r) => toItem(r as unknown as Record<string, unknown>))
  }
  ...
  async create(userId: string, data: Partial<Item>) {
    const { name, description = null, tags = null, priority = 3, completed = false, featured = false } = data
    const result = await db.execute({
      sql: `INSERT INTO item (name, description, tags, priority, completed, featured, userId)
            VALUES (?, ?, ?, ?, ?, ?, ?)`,
      args: [name!, description, tags, priority, completed ? 1 : 0, featured ? 1 : 0, userId],
    })
    return this.findOne(Number(result.lastInsertRowid), userId) as Promise<Item>
  }
